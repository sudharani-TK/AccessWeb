	...
	def perfromJobAction(String Action , String TestCaseName ,String userChoice, extentTest) {
		...
		switch (Action) {
			case 'View Details':
				...
				WebUI.delay(2)
				TestObject jobTitle=(new buildTestObj.CreateTestObj()).myJobTitleIndentifierArrayJob()
				def jobID=WebUI.getText(jobTitle)
				result=WebUI.verifyElementPresent(jobTitle, 3)
			//String[] splitAddress = jobID.split('\\.')
			//GlobalVariable.G_JobIdFromDetails=splitAddress[0]
				println jobID
				extentTest.log(LogStatus.PASS, 'Job id from details page '+ jobID)
				return result
				break
			...
			case 'View Subjobs':
				...
				WebUI.click(newJobAction)
				WebUI.delay(3)
				result=(new operations_JobsModule.GetJobRowDetails()).checkSubJobs(katalonWebDriver,'JA',extentTest)
				return result
				break
