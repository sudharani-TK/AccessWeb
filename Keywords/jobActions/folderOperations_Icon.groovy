	...
	def executeFileOperations(String Operation,String TestCaseName ,extentTest) {
		...
		switch (Operation) {
			case 'copy_icon':
				...
				if(isFilePresent){
					extentTest.log(Status.PASS, 'Verified Pasted Folder - '+ fileToCheck)
				}
			...
			//Verify notification
				...
				extentTest.log(Status.PASS, "Compressed FolderName "+compressedFolderName)
				WebUI.setText(findTestObject('FilesPage/FilesSearch_filter'), compressedFolderName)
				WebUI.sendKeys(findTestObject('JobDetailsPage/TextBx_DetailsFilter'), Keys.chord(Keys.ENTER))
				extentTest.log(Status.PASS, 'Searched for Compressed Folder ')
				...
				extentTest.log(Status.PASS, 'performed Un-Compress Operation on'+ compressedFolderName)
				WebUI.delay(3)
				WebUI.rightClick(newFileObj)
				WebUI.delay(2)
