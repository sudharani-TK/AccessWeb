...
//	WebUI.click(findTestObject('Object Repository/JobMonitoringPage/button_Yes'))
	...
	extentTest.log(Status.PASS, 'Reset the JS form ')
	...
	extentTest.log(Status.PASS, 'Select the All Fields option  ')
	...
	extentTest.log(Status.PASS, 'Added the Job Script file - Running.sh ')
	...
	for (String name1:JobFiles) {
		...
		extentTest.log(Status.PASS, 'Added the Job File -  '+ JF)
	...
	extentTest.log(Status.PASS, 'Submited the Job ')
	...
	for (String JF1:JobFiles) {
		...
		println(fileItem)
		if (fileItem) {
			extentTest.log(Status.PASS, 'Input file - '+JF1+' exists ')
		}
		//x=x+1
	}
