...
try
{
	...
	switch(userChoice)
	{
		...
		CustomKeywords.'appcomposer.Create_NewApp.create_app'(app,exec, extentTest)
			...
			extentTest.log(Status.PASS, 'Select the Test123File  created and click on ellipses')
			WebUI.click(findTestObject('AppComposer/Copy'))
			extentTest.log(Status.PASS, 'Click on Clone option')
			WebUI.delay(3)
			...
			WebUI.waitForElementPresent(findTestObject('Object Repository/AppComposer/AppSaved'), 5)
				...
				extentTest.log(Status.PASS, 'Notification Generated ' + Text)
			WebUI.delay(3)
			...
			if(result&&result1) {
				extentTest.log(Status.PASS, 'Verify cloned  copy is created and present in both App composer and Jobs page')
			}
			else {
				extentTest.log(Status.FAIL, 'Failed to verify the cloned copy')
			}
			...
			WebUI.delay(3)
			WebUI.click(findTestObject('AppComposer/Ellipses'))
			extentTest.log(Status.PASS, 'Select the app Test12File created and click on ellipses')
			WebUI.click(findTestObject('AppComposer/Copy'))
			extentTest.log(Status.PASS, 'Click on Clone option')
			WebUI.delay(3)
			WebUI.click(findTestObject('Object Repository/AppComposer/Save_ btn'))
			...
			WebUI.waitForElementPresent(findTestObject('Object Repository/AppComposer/AppSaved'), 5)
				...
				extentTest.log(Status.PASS, 'Notification Generated ' + Text3)
			...
			if(result&&result1) {
			   extentTest.log(Status.PASS, 'Verify  copy of app created and present in both App composer and Jobs page')			}
			else {
				extentTest.log(Status.FAIL, 'Failed to verify the cloned copy')
			}
			...
			*/
			...
			extentTest.log(Status.PASS, 'Click on ' + app+ ' in left navigation')
			WebUI.click(findTestObject('AppComposer/Ellipses'))
			extentTest.log(Status.PASS, 'Select the app ' + app + ' created and click on ellipses')
			WebUI.verifyElementPresent(findTestObject('AppComposer/Copy'),3)
			extentTest.log(Status.PASS, 'Click on Copy  option')
			...
			extentTest.log(Status.PASS, 'Click on apllication in left navigation')
			WebUI.click(findTestObject('AppComposer/Ellipses'))
			extentTest.log(Status.PASS, 'Select the app ' + app + ' created and click on ellipses')
			WebUI.click(findTestObject('AppComposer/Copy'))
			extentTest.log(Status.PASS, 'Click on Clone option')
			...
			WebUI.waitForElementPresent(findTestObject('Object Repository/AppComposer/AppSaved'), 5)
				...
				extentTest.log(Status.PASS, 'Notification Generated ' + Text7)
		...
		deleteobj = WebUI.modifyObjectProperty(findTestObject('AppComposer/application_name'), 'data-automation-id', 'equals', 'Newapp_1_1',true)
			...
			extentTest.log(Status.PASS, 'Rename the created app - Demonew' )
		   ...
		   editresult=WebUI.verifyElementPresent(findTestObject('AppComposer/Demonew'), 3)
			...
			{
				extentTest.log(Status.PASS, 'Verify Newapp_1 clone has been created and present in jobs page')
				extentTest.log(Status.PASS, 'Verify app has been renamed to Demonew and present on jobs page')
				extentTest.log(Status.PASS, 'Verify app has been Deleted successfully !!')
				result=true
				
			}
			else
			{
				extentTest.log(Status.FAIL, 'failing the testcase as one of the verification got failed')
				extentTest.log(Status.INFO, 'cloned app'+ cloneresult)
				extentTest.log(Status.INFO, 'Edited app'+ editresult)
				extentTest.log(Status.INFO, 'Deleted app'+ deleteresult)
				result=false
			}
			...
			WebUI.waitForElementPresent(findTestObject('Object Repository/AppComposer/AppSaved'), 5)
				...
				extentTest.log(Status.PASS, 'Notification Generated ' + Text9)
			...
			if(result&&result1) {
				 extentTest.log(Status.PASS, ' Demo_1  has been created and present in both App composer and  jobs page')		}
			else {
				extentTest.log(Status.FAIL, 'Failed to verify the cloned copy')
			}
			break
			...
			if(result)
			extentTest.log(Status.PASS, 'Verify the popup:There is a change in PAS configuration. ')
			else
			...
			extentTest.log(Status.PASS, 'Add roleid name - demo')
			...
			extentTest.log(Status.PASS, 'Click on add resources icon')
			WebUI.delay(3)
			WebUI.mouseOver(findTestObject('AppComposer/one_resource'))
			
			WebUI.click(findTestObject('AppComposer/one_resource'))
			extentTest.log(Status.PASS, 'Select the Access app as resource')
			WebUI.delay(2)
			WebUI.click(findTestObject('Access_Management/Confirm_button'))
			...
			extentTest.log(Status.PASS, 'Click on added role')
			
			WebUI.click(findTestObject('AppComposer/one_resource'))
			result=WebUI.verifyElementPresent(findTestObject('AppComposer/one_resource'), 10)
			if(result)
			extentTest.log(Status.PASS, 'Verified globally publish app showed up in access management')
			else
		...
		//	CustomKeywords.'appcomposer.Create_NewApp.create_app'(app,exec, extentTest)
			...
			/*	WebUI.click(findTestObject('AppComposer/Publishall'))
				...
				extentTest.log(Status.PASS, 'Click on  the Unpublish option')
