...
switch(jobState)
{
	...
	println(inputFolderPath)
TestObject folderPath=WebUI.modifyObjectProperty(findTestObject('Object Repository/JobDetailsPage/InputFolderPath'), 'text', 'contains',inputFolderPath , true)
		...
		switch(userChoice)
		{
			...
			case 'OutPut':
				...
				for (int i=1 ;i<listElementKey.size();i++)
				{
					RemoteWebElement ele = listElementKey.get(i)
					String myText=ele.getText()
					String eFile=jobName+".e"
					String oFile=jobName+".o"
					if(myText.contains(eFile)||myText.contains(oFile))
					{
			...
			case 'Input':
				...
				result =WebUI.verifyElementPresent(folderPath, 3)
				if(result)
				{
					extentTest.log(LogStatus.PASS, 'Job input folder listed  - '+WebUI.getText(folderPath))
				}
				String myXpathText='//div[@col-id="name"]'
